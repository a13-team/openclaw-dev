...
> - When deploying skill, `rsync --delete` must exclude `memory/`
> - When migrating workspaces, must completely preserve `memory/` and `MEMORY.md`
>
> ⛔ **Iron Law: Run `openclaw doctor` First**
> - Skill not loading, Gateway won't start, deploy failure — run `openclaw doctor` first
...
To view all currently installed skills (workspace level + managed level + bundled level), read `references/list-skills-runbook.md`.
